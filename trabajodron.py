...
if calcular:
    ...
    if res.success:
        ...
        with st.expander("⚡ Ver desglose de energía consumida por tipo de dron"):
            ...
            st.write(f"• **Tipo P en 5 GHz:** {e_p5} Wh (2 Wh/min)")
            st.write(f"• **Tipo P en 2.4 GHz:** {e_p2} Wh (2 Wh/min)")
            st.write(f"• **Tipo L en 5 GHz:** {e_l5} Wh (1 Wh/min)")
            st.write(f"• **Tipo L en 2.4 GHz:** {e_l2} Wh (1 Wh/min)")
            st.write(f"• **Reserva en Tierra:** {e_res} Wh (10 Wh fijo)")
            st.write(f"**Suma total:** {e_p5 + e_p2 + e_l5 + e_l2 + e_res} Wh")

        # --- Desglose de cobertura por dron ---
        with st.expander("🗺️ Ver desglose de cobertura alcanzada por tipo de dron"):
            ...
            st.write(f"• **Tipo P en 5 GHz:** {c_p5} km² (2 km²/min)")
            st.write(f"• **Tipo P en 2.4 GHz:** {c_p2} km² (4 km²/min)")
            st.write(f"• **Tipo L en 5 GHz:** {c_l5} km² (1 km²/min)")
            st.write(f"• **Tipo L en 2.4 GHz:** {c_l2} km² (2 km²/min)")
